...
def get_name_matches(my_df, lit_df_path, lit_name, name_append='_name'):
            ...
            if name in cont.desc_lower_2 or cont.desc_lower_2 in name:
                if name == 'actin' or cont.desc_lower_2 == 'actin':
                    if 'interacting' in name or 'interacting' in cont.desc_lower_2:
                        ...
                        my_df.at[i, f'{lit_name}{name_append}'] = name
                else:
                    my_df.at[i, f'{lit_name}{name_append}'] = name
                break
    return
...
def map_binary_colorbar(series, color='white', row_colors=None, cmap=None):
    ...
    if row_colors is None:
        ...
        row_colors = pd.concat([row_colors,
                                pd.Series(color_values.tolist(), index=series.index, name=series.name)], axis=1)
    return row_colors
